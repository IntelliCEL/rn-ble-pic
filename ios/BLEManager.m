...
- (instancetype)init
{
    if (self = [super init]) {
        ...
        LOG_TAG = @"BLE_PIC";
        NSLog(@"BLEManager created successfully.");
...
RCT_EXPORT_METHOD(
    setPeripheralName: (NSString *)name
) {
    if(isInitialized) {
...
RCT_EXPORT_METHOD(
    ...
    if(isInitialized) {
        [bleAdvertiser addService:uuid primary:primary];
    }
}
...
RCT_EXPORT_METHOD(
    ...
    if(isInitialized) {
        [bleAdvertiser addCharacteristicToService:serviceUUID characteristicUUID:uuid permissions:permissions properties:properties characteristicData:data];
    }
}
